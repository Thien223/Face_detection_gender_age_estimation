...
def unbalanced_aihub_preprocess():
	...
	os.makedirs(dir_, exist_ok=True)
	for (dir__, _, files) in os.walk(r'dataset/extracted_face'):
		for file in files:
			i += 1
			if not file.endswith('.jpg'):
				continue
			file_path = os.path.join(dir__, file)
			age, gender = file.split('.')[0].split('_')[-2:]
			age = int(age)
...
def balanced_preprocess():
	...
	os.makedirs(dir_, exist_ok=True)
	i=0
	for (dir,_,files) in os.walk(r'dataset/tarball-with-mask-balanced'):
		for file in files:
			i += 1
			print(i)
			if not file.endswith('.jpg'):
			...
			_,age, gender, filename = file_path.split('\\')
	...
	with open(os.path.join(dir_,'metadata.json'), 'w') as infor_file:
		info_file.write(str(total_info).replace('\'','\"'))
	print(f'Finished..')
	#
