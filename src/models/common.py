...
class Bottleneck(nn.Module):
	...
	def __init__(self, c1, c2, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, shortcut, groups, expansion
		...
		self.cv1 = Conv(c1, c_, k=1, s=1, p=None, g=1, act=True)
...
class BottleneckCSP(nn.Module):
	...
	def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
		...
		self.cv4 = Conv(2 * c_, c2, k=1, s=1, p=None, g=1, act=True)
...
class C3(nn.Module):
	...
	def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
		...
		c_ = int(c2 * e)  # hidden channels
		self.cv1 = Conv(c1, c_, k=1, s=1, p=None, g=1, act=True)
		self.cv2 = Conv(c1, c_, k=1, s=1, p=None, g=1, act=True)
		self.cv3 = Conv(2 * c_, c2, k=1, s=1, p=None, g=1, act=True)  # act=FReLU(c2)
		self.m = nn.Sequential(*[Bottleneck(c_, c_, shortcut, g, e=1.0) for _ in range(n)])
		# self.m = nn.Sequential(*[CrossConv(c_, c_, 3, 1, g, 1.0, shortcut) for _ in range(n)])

	def forward(self, x):
		cv1_out = self.cv1(x)
		sequential_out = self.m(cv1_out)
		cv2_out = self.cv2(x)
		concatnated = torch.cat((sequential_out,cv2_out), dim=1)
		out = self.cv3(concatnated)
		return out
...
class SPP(nn.Module):
	...
	def __init__(self, c1, c2, k=(5, 9, 13)):
		...
		self.cv1 = Conv(c1, c_, k=1, s=1, p=None, g=1, act=True)
		self.cv2 = Conv(c_ * (len(k) + 1), c2, k=1, s=1, p=None, g=1, act=True)
